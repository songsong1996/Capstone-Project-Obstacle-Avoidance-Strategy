...
int main(int argc, char **argv)
{
  ...
  // local_plan_pub_ = n.advertise<nav_msgs::Path>("/my_local_plan", 1);
  // teb_poses_pub_ = n.advertise<geometry_msgs::PoseArray>("/my_teb_poses", 100);
  ...
  // else
  //   planner = PlannerInterfacePtr(new TebOptimalPlanner(config, &obst_vector, robot_model));
  ...
  //   // PoseSequence poses=teb_band.poses();
  //   Mat back1 = Draw(obs_list, robot_position);
  //    //draw_Poses(poses,back1);
  //   imshow("tp", back1);
  //   waitKey(10);
  //   // cv::waitKey(20);
...
void CB_mainCycle(const ros::TimerEvent &e)
{
  ...
  // unsigned int i=teb_band.sizePoses();
  //cout<<teb_band.sizePoses()<<endl;
  //cout<<"teb"<<endl;
  //  PoseSequence &poses = teb_band.poses();
  //cout<<"pose1"<<endl;
  // sizePoses
  ...
  // vector<VertexPose*> poses=teb_band.poses();
  //planner.visualize();
  //  cout<<"pose1"<<endl;
  //publishLocalPlanAndPoses(teb_band, config);
  // draw_Poses(poses, back1);
