...
public:
  ...
  line_Obs(Point2f pp1, Point2f pp2)
  {
    p1.x = pp1.x;
    p1.y = pp1.y;
    p2.x = pp2.x;
...
{
  outfile.open("/home/songsong/catkin_workspace/teb_local_planner/src/teb_local_planner/src/zz1.txt",ios::out | ios::app);
   outfile2.open("/home/songsong/catkin_workspace/teb_local_planner/src/teb_local_planner/src/zz2.txt",ios::out | ios::app);
  ...
  ros::Timer cycle_timer = n.createTimer(ros::Duration(0.1), CB_mainCycle);
...
void CB_mainCycle(const ros::TimerEvent &e)
{
  ...
  if (robot_plan_message.size() != 0)
  {
    ...
    back_all = Draw(obs_list, robot_current_pose_plan);
    ...
    if (teb_optim_planner_ptr)
    {
      ...
      std::vector<ObstaclePtr> obss_tp;
      HomotopyClassPlanner planner_tp(config, &obss_tp, robot_model, visual);
      planner_tp.plan(robot_current_pose_plan, PoseSE2(teb_band.Pose(1)));
      if (planner_tp.bestTeb())
      {
        robot_current_pose_plan.x() = planner_tp.bestTeb()->teb().Pose(1).x();
        robot_current_pose_plan.y() = planner_tp.bestTeb()->teb().Pose(1).y();
        robot_current_pose_plan.theta() = planner_tp.bestTeb()->teb().Pose(1).theta();
       }
      else
      {
        ...
        robot_current_pose_plan.theta() = teb_band.Pose(1).theta();
         //  outfile<<  std::to_string(robot_current_pose_plan.x())+" "+std::to_string(robot_current_pose_plan.y()) +" "+std::to_string(robot_current_pose_plan.theta())<<endl;
    ...
    outfile<< std::to_string(robot_position_plan.x)+" "+std::to_string(robot_position_plan.y)+" "+std::to_string( robot_current_pose_plan.theta() ) <<endl;
    
      //cout << "rodist=" << cal_dist(tppppppp, robot_position_plan) << endl;

      robot_plan_message = global_plan_message(robot_position_plan, goal_point);
      draw_Poses(teb_band, back_all);
    }
    //imshow("tp1", relative_im);
    //cout << "obs_line.size()=" << obs_line.size() << endl;
    for (int i = 0; i < obs_line.size(); i++)
      line(back_all, obs_line[i].p1, obs_line[i].p2, Scalar(0, 0, 255), 2);
    imshow("tp", back_all);
    imwrite("/home/songsong/catkin_workspace/teb_local_planner/src/teb_local_planner/data"+std::to_string(image_num)+".png",back_all);
   // setMouseCallback("tp", mouseEvent);
    waitKey(10);
  }
  else
  {
...
void CB_obstacle_point(const nav_msgs::Odometry::ConstPtr &msg, const unsigned int id)
{
  ...
  Point2f tppp(obs_list[id - 1].x, obs_list[id - 1].y);
  ...
  if (id == 1)
  {
    ...
    std::vector<line_Obs> line_tp;
    line_tp.swap(obs_line);
  }
  ...
  if (cal_dist(robot_position_plan, tp_point) < 1.0 * vt / scale * 1.0)
  {
    ...
    // Point2f tppp = obs_global[id - 1].kf.predict_kt(30);
    cout << tppp.x << " " << tppp.y << " " << tp_point.x << " " << tp_point.y << endl;

    // tppp.x = (tppp.x - tp_point.x) *200 + tp_point.x;
    // tppp.y = (tppp.y - tp_point.y) *200 + tp_point.y;
    tppp.x = (tp_point.x - tppp.x) * 200 + tp_point.x;
    tppp.y = (tp_point.y - tppp.y) * 200 + tp_point.y;
    obs_line.push_back(line_Obs(Coor_Zoom(tp_point.x, tp_point.y), Coor_Zoom(tppp.x, tppp.y)));
    obst_vector_local.push_back(boost::make_shared<LineObstacle>(tp_point.x, tp_point.y, tppp.x, tppp.y));
    // line(back_all, Point(tp_point.x, tp_point.y), tppp, Scalar(0, 0, 255), 2);
    //cout << "line1" << endl;
  }
  else if (cal_dist(robot_position_plan, tp_point) < 2.0 * vt / scale * 1.0)
  {
    int idd = 1;
    double dist_min = 100;
    Point2f predict = predict_min_dist(obs_global[id - 1], idd, dist_min);
    obst_vector_local.push_back(boost::make_shared<PointObstacle>(tp_point.x, tp_point.y));
    if (dist_min < 1.0 * vt / scale * 1.0)
    {
      // if (idd < 20)
      // {
      //   Point predict2 = obs_global[id - 1].kf.predict_kt(idd + 2);

      //   // predict2.x = (tppp.x - predict.x) * 100 + predict.x;
      //   // predict2.y = (tppp.y - predict.y) * 100 + predict.y;
      //   predict2.x = (tp_point.x - tppp.x) * 200 + predict.x;
      //   predict2.y = (tp_point.y - tppp.y) * 200 + predict.y;
      //   // obs_line.push_back(line_Obs(predict, predict2));
      //   obs_line.push_back(line_Obs(Coor_Zoom(predict.x, predict.y), Coor_Zoom(predict2.x, predict2.y)));
      //   obst_vector_local.push_back(boost::make_shared<LineObstacle>(predict.x, predict.y, predict2.x, predict2.y));
      //   //cout << "line2" << endl;
      // }
    }
  }
...
void CB_RobotPosition(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  outfile2<<  std::to_string(msg->pose.pose.position.x)+" "+std::to_string(msg->pose.pose.position.y) <<endl;
...
void mouseEvent(int event, int xx, int yy, int flags, void *param)
{
  if (event == EVENT_LBUTTONDOWN)
  {
    double x = xx / scale - (all_width - rec_width) / 2.0;
    double y = -yy / scale + (all_height - rec_height) / 2.0 + rec_height;
    goal_point = Point2f(x, y);
    //cout << "button x=" << x << " y=" << y << endl;
  }
}
