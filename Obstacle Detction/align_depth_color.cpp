    ...
    for(int i=height_min;i<height_max;i++)
        for(int j=0;j<depth_align.cols;j++)
            if(depth_align.at<uint16_t>(i,j)!=0 && depth_align.at<uint16_t>(i,j)<500 )
                top_view.at<uchar>((int)(600-depth_align.at<uint16_t >(i,j)),j)=255;
    return top_view;
}

Mat depth_top2(Mat depth_align,int rows,int cols,int  height_min,int height_max)
{
    Mat top_view=Mat::zeros(rows,cols,CV_8UC1);
//    for(int i=0;i<rows;i++)
//        for(int j=0;j<cols;j++)
//            top_view.at<uchar>(i,j)=255;
    ...
    for(int i=height_min;i<height_max;i++)
        for(int j=0;j<depth_align.cols;j++)
        {
            if(depth_align.at<uchar>(i,j)<4000)
            {
                if(depth_align.at<uchar>(i,j)>max_height)
                    max_height=depth_align.at<uchar>(i,j);
            }
        }

    for(int i=height_min;i<height_max;i++)
        for(int j=0;j<depth_align.cols;j++)
            if(depth_align.at<uint16_t>(i,j)!=0 && depth_align.at<uint16_t>(i,j)<4000 )
                top_view.at<uchar>((int)(1000-depth_align.at<uint16_t >(i,j)/4),j)=255;
    return top_view;
}
...
Mat stressDepth2(Mat &image)
{
    ...
    for (int i = 0; i < image.rows; i++)
        for (int j = 0; j < image.cols; j++) {
            if(image.at<uint16_t>(i,j)>3000)result.at<uint16_t >(i,j)=0;
            else result.at<uint16_t>(i, j) = image.at<uint16_t>(i, j) *400.0;
        }
    return result;
}