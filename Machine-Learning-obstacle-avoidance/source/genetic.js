...
GeneticAlgorithm.prototype = {
	...
	createPopulation : function(){
		...
		for (var i=0; i<this.max_units; i++){
			...
			var newUnit = new synaptic.Architect.Perceptron(5,15,2);
	...
	activateBrain : function(dist_array, v,theta,vehicle){		
		...
		var inputs = [dist_array[0],dist_array[1],dist_array[2],dist_array[3],dist_array[4]];
		
		// calculate outputs by activating synaptic neural network of this bird
		var outputs = this.Population[vehicle.index].activate(inputs);
			
		// perform flap if output is greater than 0.5

		vehicle.set_dx(outputs[0]);
		vehicle.set_dy(outputs[1]);
