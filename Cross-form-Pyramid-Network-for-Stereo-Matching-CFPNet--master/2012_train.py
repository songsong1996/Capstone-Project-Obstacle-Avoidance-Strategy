...
from dataloader import KITTIloader2012 as kl2012
from models import *
...
all_left_img, all_right_img, all_left_disp, test_left_img, test_right_img, test_left_disp = kl2012.dataloader(args.datapath)
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
       ...
       for batch_idx, (imgL_crop, imgR_crop, disp_crop_L) in enumerate(TrainImgLoader):
         ...
         total_train_err += err
       print('epoch %d mean training loss = %.3f, mean training error = %.3f' %(epoch, total_train_loss/len(TrainImgLoader), total_train_err/len(TrainImgLoader)))
    ...
    for batch_idx, (imgL, imgR, disp_L) in enumerate(TestImgLoader):
           ...
           print('Iter %d test loss = %.3f, err=%.3f, time=%.2f' %(batch_idx, test_loss, err, time.time()-start_time))
           total_test_loss += test_loss
           total_test_err += err


    print('mean test loss = %.3f, mean test err = %.3f' %(total_test_loss/len(TestImgLoader), total_test_err/len(TestImgLoader)))
    #----------------------------------------------------------------------------------
