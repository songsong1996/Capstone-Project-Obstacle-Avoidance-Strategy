...
from dataloader import KITTIloader2012 as kl2015
from models import *
...
all_left_img, all_right_img, all_left_disp, test_left_img, test_right_img, test_left_disp = kl2015.dataloader(args.datapath)

TrainImgLoader = torch.utils.data.DataLoader(
         kl.myImageFloder(all_left_img,all_right_img,all_left_disp, True), 
         batch_size= 1, shuffle= True, num_workers= 1, drop_last=False)

TestImgLoader = torch.utils.data.DataLoader(
         kl.myImageFloder(test_left_img,test_right_img,test_left_disp, False), 
        ...
        for i in range(err.shape[0]):
            for j in range(err.shape[1]):
                if groundTruth_lin[i,j]!=0:
                    point=err[i,j]
                    if point>3 and point/groundTruth_lin[i,j]>0.05:
                        num=num+1
                    total=total+1
...
def adjust_learning_rate(optimizer, epoch):
    #if (epoch<100) or (epoch>150 and epoch<250) or (epoch>300 and epoch<400) or (epoch>450 and epoch<600):
    #    lr = 0.001
    #else:
    #    lr = 0.0001
    #if epoch>10000:
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
       ...
       for batch_idx, (imgL_crop, imgR_crop, disp_crop_L) in enumerate(TrainImgLoader):
        # start_time = time.time()
         ...
         #_,res=test(imgL_crop,imgR_crop,disp_crop_L)
        ...
        # print('Iter %d training loss = %.3f ,err=%.3f, time = %.2f' %(batch_idx, loss, err, time.time() - start_time))
         ...
         total_train_err += err
       print(time.strftime("%Y-%m-%d %X",time.localtime())+'   '+'epoch %d : learning rate = %2f, mean training loss = %.5f, mean training error = %.5f (%.2f sec/epoch)' %(epoch+315, lr, total_train_loss/len(TrainImgLoader), total_train_err/len(TrainImgLoader), time.time()-start_train_time))

       #SAVE
       if epoch>0:
        if epoch%5 == 0:
            savefilename = args.savemodel+'/checkpoint_'+str(epoch+315)+'.tar'
            torch.save({
            'epoch': epoch+315,
    ...
    for batch_idx, (imgL, imgR, disp_L) in enumerate(TestImgLoader):
           ...
           print('Iter %d test loss = %.5f, err=%.5f, time=%.2f' %(batch_idx, test_loss, err, time.time()-start_time))
